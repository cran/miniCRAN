...
repoPrefix <- function(type, Rversion) {
  ...
  if (
    (type == "mac.binary.el-capitan") && (numeric_version(Rversion) < "3.4")
  ) {
    warning("Type mac.binary.el-capitan only valid for R >= 3.4")
  } else if (
    (type == "mac.binary.mavericks") && (numeric_version(Rversion) >= "3.4")
  ) {
  ...
  switch(
    ...
    "mac.binary.el-capitan" = sprintf(
      "bin/macosx/el-capitan/contrib/%s",
      Rversion
    ),
    "mac.binary.leopard" = sprintf("bin/macosx/leopard/contrib/%s", Rversion),
    "mac.binary.mavericks" = sprintf(
      "bin/macosx/mavericks/contrib/%s",
      Rversion
    ),
    "mac.binary.big-sur-x86_64" = sprintf(
      "bin/macosx/big-sur-x86_64/contrib/%s",
      Rversion
    ),
    "mac.binary.big-sur-arm64" = sprintf(
...
twodigitRversion <- function(Rversion = R.version) {
  if ("simple.list" %in% is(Rversion)) {
    paste(
      Rversion$major,
      strsplit(Rversion$minor, ".", fixed = TRUE)[[1L]][1L],
      sep = "."
    )
  } else if ("R_system_version" %in% is(Rversion)) {
    paste(
      strsplit(as.character(Rversion), ".", fixed = TRUE)[[1L]][1L:2L],
      collapse = "."
    )
